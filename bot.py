...
from telegram.ext import Application, MessageHandler, CommandHandler, CallbackQueryHandler, filters, logging.basicConfig(
...
return f" Лимит по «{category}» превышен на {over}€\n(лимит {limit}€, потрачено {round(elif spent >= limit * 0.8:
...
еда, стики, здоровье, подписки, оборудование, дом и быт, одежда, дорожные расходы, красота и ПРАВИЛА КАТЕГОРИЗАЦИИ (строго соблюдай):
...
{
"type": "text",
"text": f"Сегодня {today}. Это фото чека. Извлеки КАЖДУЮ позицию отдельно, переведи },
]
else:
content = f"Сегодня {today}. Пользователь написал: «{text}». Определи категорию строго msg = client.messages.create(
model=CLAUDE_MODEL,
max_tokens=2048,
